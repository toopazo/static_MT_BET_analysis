...
class AnechaData:
    @staticmethod
    def anecha_rotor_TQP(omega, rotortype, modeltype):
        ...
        if modeltype == 'quadratic':
            ...
            if rotortype == 'KDECF245DP':
                ...
                T = aT * omega**2
                P = k * (T ** (3 / 2)) / np.sqrt(2 * rho * A) + (
                            sigma * Cd0 / 8) * (rho * A * Vtip3)
                Q = P / omega
                return [T, Q, P]
            elif rotortype == 'KDECF245TP':
                ...
                Q = P / omega
                return [T, Q, P]
            elif rotortype == 'KDECF245HP':
                sigma = solidity_nblades6
                aT = a_thrust_nblades6

                T = aT * omega**2
                P = k*(T**(3/2))/np.sqrt(2*rho*A) + (sigma*Cd0/8)*(rho*A*Vtip3)
                Q = P / omega
                return [T, Q, P]
            else:
                print(rotortype)
                raise  RuntimeError('Unrecognized rotortype')
